...
(function() {
  ...
  (function($, window, document) {
    ...
    defaults = {
      ...
      cutoffEnd: null,
      handle: false,
      cloneClass: "cloned-child",
      activeClass: "active-child",
      draggedClass: "dragged-child",
      placeholderClass: "placeholder-child",
      originalContainerClass: "original-container",
      currentContainerClass: "current-container",
      previousContainerClass: "previous-container"
    };
    Plugin = (function() {
      ...
      Plugin.prototype.errorCheck = function() {
        ...
        errors = false;
        error_msg = "drag ERROR:";
        if (options.colWidth === null) {
          $children = this.$container.children(options.selector);
          if ($children.length === 0) {
            errors = true;
            console.error("" + error_msg + " option colWidth must be specified if drag is initialized with no active children.");
          }
        }
        return !errors;
      };
      ...
      Plugin.prototype.createEvents = function() {
        ...
        $container.off("arrange").on("arrange", function(e, trigger_drop_finished) {
          if (trigger_drop_finished == null) {
            trigger_drop_finished = false;
          }
          return _this.render(false, trigger_drop_finished);
        });
        $container.off("rearrange").on("rearrange", function() {
          return _this.render(true);
        });
        $container.off("setTargetPosition").on("setTargetPosition", function() {
          return _this.setTargetPosition();
        });
        $container.off("destroy").on("destroy", function() {
          return _this.destroy();
        });
        return $container.off("shuffle").on("shuffle", function() {
      ...
      Plugin.prototype.setIdentifier = function() {
        this.identifier = "draged_container_" + Math.random().toString(36).substring(7);
      ...
      Plugin.prototype.setActiveChildren = function() {
        ...
        for (i = _j = 0, _ref = this.parsedChildren.length; 0 <= _ref ? _j < _ref : _j > _ref; i = 0 <= _ref ? ++_j : --_j) {
          ...
          if (colspan > columns && colspan > min_columns) {
            ...
            _results.push(console.error("drag ERROR: There are child elements that have a larger colspan than the minimum columns set through options.\noptions.minColumns has been set to " + colspan));
          } else {
      ...
      Plugin.prototype.setParsedChildren = function() {
        ...
        for (i = _i = 0; 0 <= total ? _i < total : _i > total; i = 0 <= total ? ++_i : --_i) {
          ...
          child = {
            i: i,
            el: $child,
            colspan: parseInt($child.attr("data-colspan")) || 1,
      ...
      Plugin.prototype.arrange = function(reparse, trigger_drop_finished) {
        ...
        if (trigger_drop_finished) {
          if (animated) {
            setTimeout((function() {
              return $container.trigger("drop-complete");
            }), animation_speed);
          } else {
            $container.trigger("drop-complete");
          }
        }
        $container.trigger("arranged");
          ...
          $container.children("." + active_class).filter(options.dragWhitelist).draggable({
            ...
            drag: function(e, ui) {
              if (!drag_timeout && !(drag_clone && delete_clone && $("." + current_container_class)[0] === $("." + original_container_class)[0])) {
                ...
                $("." + current_container_class).trigger("setTargetPosition");
                drag_timeout = true;
            ...
            stop: function() {
              ...
              if (drag_clone) {
                if (delete_clone && $("." + current_container_class)[0] === $("." + original_container_class)[0]) {
                  $clone.remove();
                  $("." + current_container_class).trigger("rearrange");
                } else {
                    $clone.removeClass(clone_class);
                    $original_container.drag($original_container.data("plugin_drag").options);
                    $current_container.drag($current_container.data("plugin_drag").options);
					check.checkIfEmpty();
					mainMenu.contructListFromDOM($('#sortable div.music-item-container'));
                }
              }
              if ($original_container[0] === $current_container[0]) {
                $current_container.trigger("rearranged", $selected);
		
				mainMenu.contructListFromDOM($('#sortable div.music-item-container'));
              } else {
                $original_container.trigger("removed", $selected);
                $current_container.trigger("added", $selected);
              }
              $original_container.trigger("arrange").removeClass(original_container_class);
              $current_container.trigger("arrange", true).removeClass(current_container_class);
              $previous_container.trigger("arrange").removeClass(previous_container_class);
          ...
          return $container.droppable({
            ...
            drop: function(e, selected) {
              ...
              if (_this.options.enableTrash) {
                ...
                $current_container.trigger("trashed", $selected);
                $selected.remove();
                $original_container.trigger("rearrange").removeClass(original_container_class);
                $current_container.trigger("rearrange").removeClass(current_container_class);
                return $previous_container.trigger("arrange").removeClass(previous_container_class);
              }
            }
          });
        }
      ...
      Plugin.prototype.setTargetPosition = function() {
        ...
        if (!options.enableTrash) {
          ...
          if ($start_container[0] !== $selected.parent()[0]) {
              previous_container_class = options.previousContainerClass;
              if ($("." + previous_container_class).data("plugin_drag").options.enableCrossDrop == true) {
                  return $("." + previous_container_class).trigger("rearrange");
              } else {
      ...
      Plugin.prototype.shuffle = function() {
        ...
        calculateShuffled = function(container, activeClass) {
          ...
          return container.each(function() {
            var items;
            items = container.find("." + activeClass);
            if (items.length) {
        ...
        if (!this.globals.dragging) {
          ...
          this.enableFeatures();
          return this.$container.trigger("rearrange");
        }
      };
      ...
      Plugin.prototype.destroy = function() {
        ...
        $container = this.$container;
        $container.off("arrange");
        $container.off("rearrange");
        $container.off("setTargetPosition");
    ...
    return $.fn[pluginName] = function(options) {
      return this.each(function() {
        ...
        old_class = (_ref = $(this).attr("class")) != null ? (_ref1 = _ref.match(/draged_container_\w+/)) != null ? _ref1[0] : void 0 : void 0;
        if (old_class) {
