...
namespace TestRig
{
    ...
    {
        private readonly WCFWebServiceJson WCFWebServiceJson;
        //private readonly WCFWebServiceJsonClient WCFWebServiceJson;

        public UserManagement()
        {
            WCFWebServiceJson = new WCFWebServiceJson();
            //WCFWebServiceJson = new WCFWebServiceJsonClient();
            InitializeComponent();
        ...
        private void btnGetUser_Click(object sender, EventArgs e)
        {
            ...
            {
                case User.UserTypeList.Admin:
                    user = WCFWebServiceJson.GetAdmin(Convert.ToInt32(txtWordpressId.Text));
                    sb = GetUserDetailsForDisplay(user);
                    break;
                case User.UserTypeList.Artist:
                    user = WCFWebServiceJson.GetArtist(Convert.ToInt32(txtWordpressId.Text));
                    sb = GetArtistDetailsForDisplay((Artist)user);
                    break;
                case User.UserTypeList.Business:
                    //user = WCFWebServiceJson.GetBusiness(Convert.ToInt32(txtWordpressId.Text));
                    //sb = GetBusinessDetailsForDisplay((BusinessUser)user);
                    sb = null;
                    break;
                default:
                    user = WCFWebServiceJson.GetCustomer(Convert.ToInt32(txtWordpressId.Text));
                    sb = GetUserDetailsForDisplay(user);
                    break;
            }
        ...
        private void btnGetAlbumDetails_Click(object sender, EventArgs e)
        {
            ...
            var sb = new StringBuilder();
            var album = (from ListItem listItem in lbArtistAlbums.SelectedItems select WCFWebServiceJson.GetArtistAlbumDetails(listItem.SelectedValue())).FirstOrDefault();
        ...
        private void btnDeleteAlbums_Click(object sender, EventArgs e)
        {
            ...
            if (WCFWebServiceJson.DeleteArtistAlbum(album, artist))
            {
        ...
        {
            var artist = WCFWebServiceJson.GetArtist(wordpressId);
            var artistAlbumCollection = WCFWebServiceJson.GetArtistAlbums(artist).ToList();
